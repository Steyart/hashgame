...
<template>
  <div>
    <div class="text-1.5xl text-white mt-10 mb-19">开奖规则</div>
    ...
    <div class="text-1.5xl text-white mt-29" @click="getName">游戏规则</div>
    <template v-if="ruleName == '哈希单双'">
      <div class="text-xs text-blue-green-ll">
        <div>
    ...
    <template v-if="ruleName == '哈希庄闲'">
      <div class="text-xs text-blue-green-ll">
        1.开奖区块的区块杂凑值<span class="text-beige">（Blockhash）</span>最后<span class="text-beige">5</span>位元数字作为牌面，如：区块哈希值（kyhl***ljl<span class="text-beige">z1a11</span>），棋牌面为”<span class="text-beige">z1a11</span>“，庄家牌面为前两位，闲家牌面为后三位，字母代表0，最终相加起来比较点数大小最后五位是z1a11，此时押注了112，庄家点数为：z+1=1，闲家点数为a+1+1=2。此时闲家＞庄家，闲家获胜
<div>2.玩家押注的个位数为2，押的闲家，所以玩家获胜，按照赔率进行赔付</div>
3.庄闲获胜赔率为1：1.95，和的赔率为1：8
      </div>
    </template>
    <template v-if="ruleName == '哈希和值大小'">
      <div class="text-xs text-blue-green-ll">
        1.查看本次转账区块和该区块的下一块区块的区块哈希值的最后一个
数字，并计算其和值（小数点后的数字视为无效数字）
<div>2.最后和值相加后个位数为<span class="text-beige">0,1,2,3,4</span>辨识为小</div>
3.最后和值相加后个位数为<span class="text-beige">5,6,7,8,9</span>辨识为大
      </div>
    </template>
    <div class="text-1.5xl text-white mt-20">注意事项</div>
    <div class="text-xs text-blue-green-ll">
      <div>
